...
function showGoogleDriveConnected() {
    ...
    if (statusEl) {
        ...
        statusEl.innerHTML = '<i class="fas fa-check-circle"></i><span>Đã kết nối Google Drive</span>';
    }
    if (loginBtn) loginBtn.style.display = 'none';
    if (logoutBtn) logoutBtn.style.display = 'flex';
    if (syncActions) syncActions.style.display = 'grid';
    
    // Show last sync time
    const lastSync = localStorage.getItem('volearn-gdrive-lastsync');
    if (lastSyncEl && lastSync) {
        lastSyncEl.textContent = `Đồng bộ lần cuối: ${new Date(lastSync).toLocaleString('vi-VN')}`;
    }
}
